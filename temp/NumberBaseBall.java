...
public class numberbaseball {
	...
	public static void start() {
		...
		while (currentStep == 1) {
			...
			System.out
					.println("check, S/B, count : " + ask[0] + ask[1] + ask[2] + ", " + S + "/" + B + ", " + (++count));
			if (B > 0) {
				for (int i = 0; i < 3; i++) {
					ask2[i] = ask[i];
				}
		...
		while (true) {
			...
			while (true) {
				...
				if (ask[0] == ask[1] || ask[0] == ask[2] || ask[1] == ask[2]) continue;
				break;
			}
